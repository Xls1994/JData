...
user_after_path='data/User.csv'
def parse_user(user_path,user_after_path):
    dataFrame =pd.read_csv(user_path,encoding='gbk')
    dataFrame.to_csv(user_after_path,encoding='utf-8',index=False)

user_data_reg_path ='data/User_reg.csv'
user_data_final_dumpy_path='data/User_final.csv'
#处理用户数据
def deal_with_user_data(user_after_path,user_data_reg_path,user_final_dumpy_path):
    # 将用户的注册日期改为距离预测日期的时间长度，又称为注册时长
    csvfile = file(user_data_reg_path, 'w')
    writer = csv.writer(csvfile)
    reader = open(user_after_path)
    ...
    for line in reader.readlines():
        ...
        if line[4] != '':
            ...
            writer.writerow((line[0], line[1], line[2], line[3], 0, 0, 0))

    # 1  79  36-45岁  2  2   69  74  79
    # 将年龄、性别、等级dummies化
    user_data = pd.read_csv(user_data_reg_path)
    newdf = pd.get_dummies(user_data, columns=["age", "sex", 'user_lv_cd'], dummy_na=True)
    print newdf.head()
    # newdf =pd.concat([user_data['user_id'], newdf], axis=1)
    newdf.to_csv(user_final_dumpy_path, index=False)
comment_data_path ='data/JData_Comment.csv'
comment_data_final_path='data/JData_Comment_final.csv'
#处理评论数据
def deal_with_comment_data(comment_data_path,comment_data_final_path):
    comment_data = pd.read_csv(comment_data_path, parse_dates=[0])
    #  原 dt  sku_id  comment_num  has_bad_comment  bad_comment_rate
    # 转换为 sku_id  avg_comment_num  avg_have_bad_comment_sku_mean avg_bad_comment_rate
    ...
    data.to_csv(comment_data_final_path,index=False
                )

# 产生48维特征
def fetch_feature(sample_filename, feature_filename, item_brand={}, code='_one'):
    # ['user_id', 'sku_id', 'time', 'model_id', 'type', 'cate', 'brand']
    reader = csv.reader(file(sample_filename, 'r'))
    csvfile = file(feature_filename, 'w')
    writer = csv.writer(csvfile)
    writer.writerow(
        ('user_id', 'sku_id', 'user_item_click' + code,
         'user_click' + code, 'usr_item_hide' + code, 'user_hide' + code,
         'usr_item_shop_basket' + code, 'user_basket' + code,
         'usr_item_shop' + code, 'user_buy' + code,
         'item_num' + code,
         'len_item_user' + code,
         ...
         'buy_catogry_ratio' + code, 'click_buy_user_ratio' + code,
         'basket_buy_ratio' + code, 'click_basket' + code,
         'basket_buy_user_ratio' + code,
         'ratio_hide_buy' + code,
         'ratio_click_basket' + code, 'click_catogry_ratio' + code,
         'basket_catogry_ratio' + code,
         'catogry_click_buy' + code,
         'catogry_basket_buy' + code,
         'catogry_hide_buy' + code, 'comm_item_ratio' + code,
         'comm_brand_buy_ratio' + code, 'eraliest_time' + code,
         'latest_time' + code,
         'user_basket_delete' + code, 'user_basket_item_brand_delete' + code,
         'catogery_basket_delete' + code,
         'item_basket_delete' + code,
         'user_view' + code, 'user_view_item_brand' + code,
         'catogery_view' + code, 'item_view' + code
    ...
    for k in user_item_pair:
        ...
        writer.writerow((k[0], k[1], user_item_click[k],
            user_click[k[0]],
            usr_item_hide[k],
            user_hide[k[0]],
            usr_item_shop_basket[k],
            user_basket[k[0]],
            usr_item_shop[k],
            user_buy[k[0]],
            item_num[k[1]],
            len(item_user[k[1]]),
            len(user_buy_brand[k[0]]),
            user_buy_item_brand[(k[0], item_brand[k[1]])],
            len(user_item_num[k[0]]),
            len(user_brand[k[0]]),
            user_click_item_brand[(k[0], item_brand[k[1]])],
            user_basket_item_brand[(k[0], item_brand[k[1]])],
            catogery_click[item_brand[k[1]]],
            catogery_hide[item_brand[k[1]]],
            catogery_basket[item_brand[k[1]]],
            catogery_buy[item_brand[k[1]]], item_click[k[1]],
            item_hide[k[1]],
            item_basket[k[1]],
            buy_catogry_ratio,
            click_buy_user_ratio,
            basket_buy_ratio,
            click_basket,
            basket_buy_user_ratio,
            ratio_hide_buy,
            ratio_click_basket,
            click_catogry_ratio,
            basket_catogry_ratio,
            catogry_click_buy,
            catogry_basket_buy,
            catogry_hide_buy,
            comm_item_ratio,
            comm_brand_buy_ratio,
            eraliest_time,
            latest_time,
            user_basket_delete[k[0]],
            user_basket_item_brand_delete[(k[0], item_brand[k[1]])],
            catogery_basket_delete[item_brand[k[1]]], item_basket_delete[k[1]],
            user_view[k[0]],
            user_view_item_brand[(k[0], item_brand[k[1]])],
            catogery_view[item_brand[k[1]]],
            item_view[k[1]]
                         ))
...
if __name__=='__main__':
    # parse_user(user_path,user_after_path)
    deal_with_user_data(user_after_path,user_data_reg_path,user_data_final_dumpy_path)