...
def get_comments_product_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path):
        ...
        commnets =pd.read_csv(dump_path)
    else:
...
def get_accumulate_user_feat(start_date, end_date):
    '''
    计算各个行为对于‘点击’的比率，然后保存csv
    会出现inf，需要处理！(这个代码没有处理)
    :param start_date:
    :param end_date:
    :return: actions
    '''
    ...
    if os.path.exists(dump_path):
        ...
        actions.to_csv(dump_path,index=False)
    print 'finish user ration computation....'
        # pickle.dump(actions, open(dump_path, 'w'))
    return actions
...
if __name__ == '__main__':
    ...
    # get_action_feat(train_start_date,train_end_date)
